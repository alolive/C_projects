...
property_t *get_gconf_properties( property_t *current_prop, const char *fqdn ) {
  ...
  char source[SOURCE_PROP_MAX_LENGTH] = "(Gconf)";
  FILE *snippet_gconf = NULL;
  ...
  sprintf( filename, "%s/%s%s", SNIPPET_DIR, fqdn, ".gjson" );
  snippet_gconf = fopen( filename, "r" );
  if ( snippet_gconf == NULL ) { return( NULL ); }
  
  /* parse snippet line by line */
  while (( line_size = getline( &line_buffer, &length, snippet_gconf )) != -1 ) {
